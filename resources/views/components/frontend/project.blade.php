...
<a x-data="{ skill: {{ json_encode($project->skill) }} }" href="{{ $project->project_url }}" target="_blank"
    :class="selectedSkill == 'all' || selectedSkill == skill.id ? 'block' : 'hidden'"
    ...
    <div class="flex items-center px-6 py-3 bg-gray-900">
        ...
        <h1
            class="mx-3 text-lg font-semibold text-white group-hover:text-light-tale-500 dark:group-hover:text-dark-navy-100">
            {{ $project->skill->name }}</h1>
    </div>

    <div class="px-6 py-4">
        <h1
            class="text-xl font-semibold text-gray-800 dark:text-white group-hover:text-light-tale-500 dark:group-hover:text-dark-navy-100">
            {{ $project->name }}</h1>
        <div
